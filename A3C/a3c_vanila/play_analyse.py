...
def init_saliency_map(args, agent, history, first_frame=0, prefix='QF_', resolution=75, save_dir='./movies/', env_name='Breakout-v0'):
    ...
    _, _, load_model_backprop ,_= build_network(agent.observation_shape, agent.action_space_n)
    ...
    _, _,load_model_grad_cam,_ = build_guided_model(agent.observation_shape, agent.action_space_n)

    load_model_backprop.load_weights(args.load_network_path)
    ...
    backprop_actor = init_guided_backprop(load_model_backprop,"dense_6")
    backprop_critic = init_guided_backprop(load_model_backprop,"dense_5")
    cam_actor = init_grad_cam(load_model_backprop, "convolution2d_4")
    cam_critic = init_grad_cam(load_model_backprop, "convolution2d_4", False)
    guidedBackprop_actor = init_guided_backprop(load_model_grad_cam,"dense_9")
    guidedBackprop_critic = init_guided_backprop(load_model_grad_cam,"dense_8")
    ...
    for i in range(num_frames):#total_frames): #num_frames
        ...
        if ix < total_frames: # prevent loop from trying to process a frame ix greater than rollout length
            ...
            Cam_heatmap = grad_cam(gradCAM_actor, frame, action)
    ...
    fig_array[0,0] = normalization(history_gradients_actor, history, "gdb")
    fig_array[0,1] = normalization(history_gradients_critic, history, 'gdb')
    fig_array[1,0] = normalization(history_gradCam_actor, history, "cam", GDB_actor=0)
    fig_array[1,1] = normalization(history_gradCam_critic, history, 'cam')
    fig_array[2,0] = normalization(history_gdb_actor, history, "gdb")
    fig_array[2,1] = normalization(history_gdb_critic, history, 'gdb')
    fig_array[3,0] = normalization(history_guidedGradCam_actor, history, "cam", GDB_actor=0)
