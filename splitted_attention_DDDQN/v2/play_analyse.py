...
def init_saliency_map(args, agent, history, first_frame=0, num_frames=1000, prefix='QF_', resolution=75, save_dir='./movies/', env_name='Breakout-v0'):
    ...
    backprop_fn = init_guided_backprop(visualization_network_model,"merge_5")
    if args.duel_visual:
        backprop_fn_advatage = init_guided_backprop(visualization_network_model,"merge_6")
        fig_array = np.zeros((2,667,84,84,3))
    else:
        fig_array = np.zeros((1,667,84,84,3))
    for i in range(667):#total_frames): #num_frames
        ix = first_frame+i
...
def normalization(gbp_heatmap, history):
    ...
    gbp_heatmap*= 0.1 
    #print("heatmap",gbp_heatmap[0,:,:,0])
    #frame=frame[0,:,:,0]
    #print(frame)

    # clip to [0, 1]
    #gbp_heatmap += 0.5
    gbp_heatmap = np.clip(gbp_heatmap, -1, 1)
    gbp_heatmap_pic1 = gbp_heatmap[:,0,:,:,0]
    gbp_heatmap_pic2 = gbp_heatmap[:,0,:,:,0]
    all_unproc_frames = history['un_proc_state'].copy()
    frame=np.zeros((667,84,84,3))
    ...
    proc_frame = overlap(frame,gbp_heatmap_pic1)
...
def overlap(frame,gbp_heatmap):
    color_neg = [0.0, 1.0, 0.0]
    color_pos = [1.0, 0.0, 0.0]
    color_chan = np.ones((667,84,84,2),dtype=gbp_heatmap.dtype)
    alpha = 0.5
    #beta = 0.25
...
def play_game(args, agent, env, total_episodes=1):
    ...
    history = { 'state': [], 'un_proc_state' : [], 'action': [], 'gradients':[], 'gradients_duel_adv':[],'movie_frames':[]}
    rewards = []
