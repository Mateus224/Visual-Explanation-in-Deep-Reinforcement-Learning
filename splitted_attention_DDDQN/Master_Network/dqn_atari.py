...
def get_output_folder(args, parent_dir, env_name, task_name):
    ...
    parent_dir = parent_dir + '-run{}'.format(66)#experiment_id) + '-' + task_name
    if not os.path.exists(parent_dir):
        os.makedirs(parent_dir)
        print('===== Folder did not exist; creating... %s'%parent_dir)
    else:
...
def main():
    ...
    parser.add_argument('--convert', default=False, dest='convert', action='store_true', help='convert and stor the model')
