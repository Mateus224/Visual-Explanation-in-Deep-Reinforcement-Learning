        ...
        if mode == "linear":
            ...
            if not(args.recurrent):
                ...
                if mode == "duel":
                    ...
                    ##################################
                    value_hidden =Bidirectional(LSTM(512, return_sequences=True,  name = 'value_hidden', stateful=False, input_shape=(args.num_frames, 512)), merge_mode='sum') (hidden_input) #Dense(512, activation = 'relu', name = 'value_fc')(all_outs)
                    value_hidden_out=all_outs = Bidirectional(LSTM(512, return_sequences=True,  name = 'action_hidden_out', stateful=False, input_shape=(args.num_frames, 512)), merge_mode='sum') (value_hidden)
                    value = TimeDistributed(Dense(1, name = "value"))(value_hidden_out)
                    
                    action_hidden =Bidirectional(LSTM(512, return_sequences=True,  name = 'action_hidden', stateful=False, input_shape=(args.num_frames, 512)), merge_mode='sum') (hidden_input) #Dense(512, activation = 'relu', name = 'value_fc')(all_outs)
                    action_hidden_out=all_outs = Bidirectional(LSTM(512, return_sequences=True,  name = 'action_hidden_out', stateful=False, input_shape=(args.num_frames, 512)), merge_mode='sum') (action_hidden)
                    action = TimeDistributed(Dense(18, name = "action"))(action_hidden_out)



                    #Bidirection for a(s,a) and v(s)
                    #################################
                    #value_hidden = Dense(512, activation = 'relu', name = 'value_fc')(hidden_input)
                    #value_bi = Bidirectional(LSTM(1, return_sequences=True,  name = 'value_hidden_out', stateful=False, input_shape=(args.num_frames, 512)), merge_mode='sum')(value_hidden)
                    #value = Bidirectional(LSTM(1, return_sequences=True,  name = 'value_hidden_out', stateful=False, input_shape=(args.num_frames, 1)), merge_mode='sum')(value_bi)
                
                    #action_hidden = Dense(512, activation = 'relu', name = 'action_fc')(hidden_input)
                    #action_bi = Bidirectional(LSTM(18, return_sequences=True,  name = 'value_hidden_out', stateful=False, input_shape=(args.num_frames, 512)), merge_mode='sum')(action_hidden)
                    #action = Bidirectional(LSTM(18, return_sequences=True,  name = 'value_hidden_out', stateful=False, input_shape=(args.num_frames, 18)), merge_mode='sum')(action_bi)
                    ...
                    # attention  A(s)
                    sent_representation_as = merge([value, attention_vs], mode='mul')
                    ...
                    #attention_mean_as = RepeatVector(18)(attention_as)
                    #attention_mean_as = Permute([2, 1])(attention_mean_as)

                    attention_as = RepeatVector(18)(attention_as)
                    attention_as = Permute([2, 1])(attention_as)
                    sent_representation_vs =merge([action, attention_as], mode='mul')

                    action_mean = Lambda(lambda x: tf.reduce_mean(x, axis = 2, keep_dims = True), name = 'action_mean')(sent_representation_as) 
                    output = Lambda(lambda x: x[0] + (x[1] - x[2]), name = 'duel_output')([sent_representation_vs, sent_representation_as, action_mean])
...
class DQNAgent:
    ...
    def __init__(self, args, num_actions):
        ...
        self.input_shape = (args.frame_height, args.frame_width, args.num_frames)
        ...
        self.q_network = create_model(self.input_shape, self.num_actions, self.net_mode, args, "QNet")
        self.target_network = create_model(self.input_shape, self.num_actions, self.net_mode, args, "TargetNet")
    ...
    def compile(self, optimizer = None, loss_func = None):
        ...
        with tf.variable_scope("Loss"):
            ...
            qa_value = merge([qa_value, action_mask], mode = 'mul', name = "mul")#multiply,
            qa_value = Lambda(lambda x: tf.reduce_sum(x, axis=1, keep_dims = True), name = "sum")(qa_value)

        self.final_model = Model(input = [state, action_mask], output = qa_value)
