...
NUM_REPLAY_MEMORY = 120000  # Number of replay memory the agent uses for training
BATCH_SIZE = 32  # Mini batch size
TARGET_UPDATE_INTERVAL = 40000  # The frequency with which the target network is updated
TRAIN_INTERVAL = 4  # The agent selects 4 actions between successive updates
LEARNING_RATE = 0.00025  # Learning rate used by RMSProp
MOMENTUM = 0.95  # Momentum used by RMSProp
MIN_GRAD = 0.01  # Constant added to the squared gradient in the denominator of the RMSProp update
SAVE_INTERVAL = 100000  # The frequency with which the network is saved
NO_OP_STEPS = 20  # Maximum number of "do nothing" actions to be performed by the agent at the start of an episode
LOAD_NETWORK = True
TRAIN = False
...
class Agent():
    ...
    def build_network(self):
        ...
        model.compile(loss='mse', optimizer=Adam(lr=0.000001)) #loss=tf.keras.losses.Huber(delta = 1.0),  metrics=['mae', root_mean_squared_log_error]
    ...
    def train_network(self):
        ...
        target_q_values = self.q_network.predict(np.array(next_state_batch) / 255.0, batch_size=32)
        ...
        self.q_network.fit(np.array(state_batch), target_q_values, nb_epoch=1, verbose=0, batch_size=32)
